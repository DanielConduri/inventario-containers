...
function generarPDFInformePrestamo2(contenido) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let asunto = `Por medio de la presente, yo, ${
      contenido.prestamo.str_prestamo_persona_nombre
    }, con número de cédula ${
      contenido.prestamo.int_prestamo_persona_id
    ...
    const addHeaderAndFooter = () => {
      ...
      doc.text("SOLICITUD DE PRÉSTAMO "+ contenido.prestamo.str_prestamo_tipo.toUpperCase() , titleX, titleY, {
        align: "right",
      });
      doc.moveDown(0.5);
    ...
    doc.moveDown(2);
    doc.font("Times-Bold").fontSize(sizeLetra).fillColor(colorTexto);
    doc.text("Objeto de la Investigación o Trabajo:", {});
    doc.font("Times-Roman").fontSize(sizeLetra).fillColor(colorTexto);
    ...
    doc.moveDown(2);
    doc.font("Times-Bold").fontSize(sizeLetra).fillColor(colorTexto);
    doc.text("Fecha y Hora de Uso/Salida del Equipo:", {});
    doc.font("Times-Roman").fontSize(sizeLetra).fillColor(colorTexto);
    doc.text( contenido.fecha.dt_fecha_prestamo.toLocaleDateString("es-ES", options) + " " + contenido.horario.tm_horario_hora_inicio, { continued: true, } );
    doc.moveDown(2);
    doc.font("Times-Bold").fontSize(sizeLetra).fillColor(colorTexto);
    doc.text("Características del Equipo o Material:", {});
    doc.moveDown(1);
...
export default generarPDFInformePrestamo2;
