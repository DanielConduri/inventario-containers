...
export class AgregarCentrosComponent implements OnInit, OnDestroy {
  ...
  facul() {
    ...
    this.srvCentros.getTypeAc(this.elementCenterAc).
      ...
      .subscribe({
        ...
        Swal.close()
          ...
          this.facultad.facult = this.srvCentros.datosFacultad.map((fac: any) => fac.strNombre)
          this.facultad.cod = this.srvCentros.datosFacultad.map((cod: any) => cod.strCodFacultad)
  ...
  getF(e: any) {
    ...
    if (selectedId) {  
      this.myFormAc.get('codigo_facultad')?.setValue(selectedId) 
      this.elementCenterAc.codfacultad = this.myFormAc.value.codigo_facultad
  ...
  carrer(){
    ...
    this.srvCentros.getCarrera(this.elementCenterAc)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (data:any)=>{
        Swal.close()
        ...
        this.carrera.carrera = this.srvCentros.datosCarrera.map((car: any) => car.strNombre)
        this.carrera.cod = this.srvCentros.datosCarrera.map((car: any) => car.strCodCarrera)
  ...
  getC(e: any){
    ...
    if (selectedId) {  
      this.myFormAc.get('codigo_carrera')?.setValue(selectedId) 
      this.elementCenterAc.codcarrera = this.myFormAc.value.codigo_carrera
      ...
      this.myFormP.value.codigo_dependencia = 0
      this.myFormP.value.codigo_ubicacion = 0
  ...
  ubi(){
    this.srvCentros.getTypeAd(this.elementCenterAd).
      pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data: any) => {
          ...
          this.dependencia.depen = this.srvCentros.datosDependencia.map((depe: any) => depe.procesoDependencia)
          this.dependencia.cod = this.srvCentros.datosDependencia.map((cod:any) => cod.dependenciaId)
        },
        error: (err) => {
          console.log('error ->', err);
        }
  ...
  depende(e: any){
    ...
    const selectedId: number = parseInt(selectedOption.id); 
    if (selectedId) {  
      this.myFormAd.get('codigo_dependencia')?.setValue(selectedId) 
      this.elementCenterAd.dependenciaId = this.myFormAd.value.codigo_dependencia
    ...
    this.srvCentros.getDependencia(this.elementCenterAd)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (data: any) =>{
        Swal.close()
        ...
        this.ubicacion.ubica = this.srvCentros.datosUbicacion.map((ubi:any) => ubi.procesoDescripcion)
        this.ubicacion.cod = this.srvCentros.datosUbicacion.map((cod: any) => cod.procesoId)
      }, error: (err) => {
        console.log('error ->', err);
      }
    })
  ...
  getUbi(e:any){
    ...
    const selectedId: number = parseInt(selectedOption.id); 
    if (selectedId) {  
      this.myFormAd.get('codigo_ubicacion')?.setValue(selectedId) 
      this.elementCenterAd.ubicacion = this.myFormAd.value.codigo_ubicacion
  ...
  send() {
    ...
    const sendDataAc = this.myFormP.value
      Swal.fire({
        ...
        if(result.isConfirmed){
          this.srvCentros.postFile(sendDataAc)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next:(rest)=>{
